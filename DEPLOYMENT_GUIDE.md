# 🚀 EC2 Deployment Guide - Complete Start to End

**Domain**: test.plotchoice.com  
**Time Required**: ~70 minutes (includes validation)  
...
⚠️ **PREREQUISITE: Mount EBS Volumes FIRST**

```bash
# Step 1: MANDATORY - Mount EBS volumes (interactive)
sudo bash scripts/setup-ebs.sh
# You'll be asked to identify device names (e.g., /dev/nvme1n1, /dev/nvme2n1)

# Step 2: Then run Docker + Certbot setup
sudo bash scripts/setup-all.sh your-domain.com your-email@example.com

# Example:
sudo bash setup-all.sh test.plotchoice.com admin@example.com
```

**What happens:**
1. **setup-ebs.sh** (Step 1) - Mounts your 2 EBS volumes with persistent fstab (5 min) ⚠️ **MUST DO FIRST**
2. **setup-all.sh** (Step 2) - Installs Docker, docker-compose, Certbot, SSL (5-10 min)
3. Remaining steps in this guide (Steps 4-5+)

**Scripts available in `scripts/` folder**:
- `setup-ebs.sh` - Mount EBS volumes (⚠️ RUN FIRST)
- `setup-all.sh` - Complete Docker + Certbot setup
- `setup-docker.sh` - Docker only
- `setup-certbot.sh` - Certbot only

📖 **Detailed instructions**: See [SETUP-SCRIPTS.md](./SETUP-SCRIPTS.md)

---

## 🧪 Docker Compose Dry-Run Commands

Before you start: Docker Compose has built-in validation commands (dry-run equivalent):

| Command | Purpose | When to Use |
|---------|---------|------------|
| `docker compose config` | Validates YAML syntax & shows configuration | Before Step 5 - test setup |
| `docker compose pull` | Download images without starting containers | Before Step 5 - test images |
| `docker compose config --services` | List all services that will be deployed | Quick validation |
| `docker ps` | Show running containers | After deployment |
| `docker compose ps` | Show status of all services | Quick health check |

**No actual "--dry-run" flag exists for docker compose up**, but using `docker compose config` and `docker compose pull` accomplishes the same validation before deployment.
...
- [ ] 2 EBS volumes attached (postgres: 20GB, certs: 5GB)
- [ ] DNS A record pointing to EC2 IP
- [ ] SSH access to EC2 instance
- [ ] Security Groups open for ports 80, 443, 22

**Files ready on your local machine**:
- [ ] `docker-compose.yml` from docker-project/
- [ ] `nginx/conf.d/default.conf` from docker-project/
- [ ] `.env.example` from docker-project/
- [ ] README.md from docker-project/

---

## 📁 Files to Gather Before Starting

**Critical files you'll need to copy to EC2**:

```
From your local machine, locate these files:
real-state/
└── docker-project/
    ├── docker-compose.yml           ← You'll copy this
    ├── .env.example                 ← You'll copy this
    ├── README.md                    ← You'll copy this
    └── nginx/
        └── conf.d/
            └── default.conf         ← You'll copy this
```

**How to gather them**:
1. Navigate to your local `real-state/docker-project/` folder
2. Ensure these 4 items exist:
   - `docker-compose.yml` (file)
   - `.env.example` (file)
   - `nginx/conf.d/default.conf` (file - in nginx subdirectory)
   - `README.md` (file)

**You do NOT need**:
- ❌ RE-Backend/ folder (images are pre-built)
- ❌ RE-FrontEnd/ folder (images are pre-built)
- ❌ Scripts or backup folders
- ❌ .git folder

**During Step 4**, you'll copy these files to EC2 using either:
- `git clone` (full repository), then navigate to docker-project/
- `scp` commands (copy individual files - see Step 4 for commands)
